...
async def handle_airflow(node, creds, owner_id, ctx, nlog, **kw):
    ...
    operation = props.get("operation", "trigger")   # default = trigger (backward compat)
    ...
    nlog.section(f"Airflow — operation={operation}")
    ...
    if operation == "list_dags":
        limit = int(props.get("limit", 100))
        tags  = props.get("tags") or []
        nlog.info(f"Listing DAGs (limit={limit})")
        dags = await _t(connector.list_dags, limit=limit, tags=tags or None)
        nlog.ok(f"Found {len(dags)} DAGs")
        for d in dags[:20]:
            nlog.info(f"  {d.get('dag_id','?'):<40s}  paused={d.get('is_paused')}  schedule={d.get('schedule_interval')}")
        return {"dags": dags, "count": len(dags)}

    elif operation == "get_dag":
        if not dag_id: raise ValueError("get_dag requires dag_id")
        nlog.info(f"Getting DAG: {dag_id}")
        dag = await _t(connector.get_dag, dag_id)
        nlog.ok(f"DAG {dag_id}: paused={dag.get('is_paused')}  schedule={dag.get('schedule_interval')}")
        return dag

    elif operation == "get_dag_details":
        if not dag_id: raise ValueError("get_dag_details requires dag_id")
        nlog.info(f"Getting DAG details: {dag_id}")
        details = await _t(connector.get_dag_details, dag_id)
        nlog.ok(f"Got details for {dag_id}")
        return details

    elif operation == "get_dag_source":
        if not dag_id: raise ValueError("get_dag_source requires dag_id")
        nlog.info(f"Getting source for DAG: {dag_id}")
        dag      = await _t(connector.get_dag, dag_id)
        file_tok = dag.get("file_token", "")
        if not file_tok: raise ValueError(f"DAG {dag_id!r} has no file_token")
        source = await _t(connector.get_dag_source, file_tok)
        nlog.ok(f"Source retrieved ({len(source)} chars)")
        return {"dag_id": dag_id, "source": source, "length": len(source)}

    elif operation == "pause":
        if not dag_id: raise ValueError("pause requires dag_id")
        nlog.info(f"Pausing DAG: {dag_id}")
        result = await _t(connector.pause_dag, dag_id)
        nlog.ok(f"DAG {dag_id} paused")
        return {"dag_id": dag_id, "is_paused": True, **result}

    elif operation == "unpause":
        if not dag_id: raise ValueError("unpause requires dag_id")
        nlog.info(f"Unpausing DAG: {dag_id}")
        result = await _t(connector.unpause_dag, dag_id)
        nlog.ok(f"DAG {dag_id} unpaused")
        return {"dag_id": dag_id, "is_paused": False, **result}

    elif operation == "delete_dag":
        if not dag_id: raise ValueError("delete_dag requires dag_id")
        nlog.warn(f"Deleting DAG: {dag_id}")
        status = await _t(connector.delete_dag, dag_id)
        nlog.ok(f"DAG {dag_id} deleted (HTTP {status})")
        return {"dag_id": dag_id, "deleted": True, "status_code": status}

    elif operation == "list_dag_warnings":
        nlog.info(f"Listing DAG warnings{' for ' + dag_id if dag_id else ''}")
        warnings = await _t(connector.list_dag_warnings, dag_id or None)
        nlog.ok(f"Found {len(warnings)} warning(s)")
        for w in warnings:
            nlog.warn(f"  {w.get('dag_id','?')}: {w.get('message','?')}")
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        if not dag_id: raise ValueError("list_runs requires dag_id")
        limit  = int(props.get("limit", 10))
        states = props.get("states") or None
        nlog.info(f"Listing runs for {dag_id} (limit={limit})")
        runs = await _t(connector.list_dag_runs, dag_id, limit=limit, states=states)
        nlog.ok(f"Found {len(runs)} run(s)")
        for r in runs:
            nlog.info(f"  {r.get('dag_run_id','?'):<40s}  state={r.get('state')}  start={r.get('start_date','')[:19]}")
        return {"dag_id": dag_id, "runs": runs, "count": len(runs)}

    elif operation == "get_run":
        if not dag_id: raise ValueError("get_run requires dag_id")
        dag_run_id = props.get("dag_run_id", "")
        if not dag_run_id: raise ValueError("get_run requires dag_run_id")
        nlog.info(f"Getting run {dag_run_id} for DAG {dag_id}")
        run = await _t(connector.get_dag_run, dag_id, dag_run_id)
        nlog.ok(f"Run state: {run.get('state')}")
        return run

    elif operation == "clear_run":
        if not dag_id: raise ValueError("clear_run requires dag_id")
        dag_run_id = props.get("dag_run_id", "")
        dry_run    = bool(props.get("dry_run", False))
        if not dag_run_id: raise ValueError("clear_run requires dag_run_id")
        nlog.info(f"Clearing run {dag_run_id} (dry_run={dry_run})")
        result = await _t(connector.clear_dag_run, dag_id, dag_run_id, dry_run=dry_run)
        nlog.ok(f"Run cleared{'(dry run)' if dry_run else ''}")
        return {"dag_id": dag_id, "dag_run_id": dag_run_id, "dry_run": dry_run, **result}

    elif operation == "delete_run":
        if not dag_id: raise ValueError("delete_run requires dag_id")
        dag_run_id = props.get("dag_run_id", "")
        if not dag_run_id: raise ValueError("delete_run requires dag_run_id")
        nlog.warn(f"Deleting run {dag_run_id}")
        status = await _t(connector.delete_dag_run, dag_id, dag_run_id)
        nlog.ok(f"Run deleted (HTTP {status})")
        return {"dag_id": dag_id, "dag_run_id": dag_run_id, "deleted": True}

    elif operation == "update_run_state":
        if not dag_id: raise ValueError("update_run_state requires dag_id")
        dag_run_id = props.get("dag_run_id", "")
        state      = props.get("state", "")
        if not dag_run_id: raise ValueError("update_run_state requires dag_run_id")
        if state not in ("queued", "success", "failed"):
            raise ValueError(f"update_run_state: state must be queued|success|failed, got {state!r}")
        nlog.warn(f"Setting run {dag_run_id} state → {state}")
        result = await _t(connector.update_dag_run_state, dag_id, dag_run_id, state)
        nlog.ok(f"Run state updated to {state}")
        return {"dag_id": dag_id, "dag_run_id": dag_run_id, "state": state, **result}
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        if not dag_id: raise ValueError("get_task requires dag_id")
        dag_run_id = props.get("dag_run_id", "")
        task_id    = props.get("task_id", "")
        if not dag_run_id: raise ValueError("get_task requires dag_run_id")
        if not task_id:    raise ValueError("get_task requires task_id")
        nlog.info(f"Getting task {task_id} in run {dag_run_id}")
        ti = await _t(connector.get_task_instance, dag_id, dag_run_id, task_id)
        nlog.ok(f"Task state: {ti.get('state')}")
        return ti

    elif operation == "update_task_state":
        if not dag_id: raise ValueError("update_task_state requires dag_id")
        dag_run_id = props.get("dag_run_id", "")
        task_id    = props.get("task_id", "")
        state      = props.get("state", "")
        dry_run    = bool(props.get("dry_run", False))
        if not dag_run_id: raise ValueError("update_task_state requires dag_run_id")
        if not task_id:    raise ValueError("update_task_state requires task_id")
        valid_states = ("success", "failed", "skipped", "up_for_retry", "queued")
        if state not in valid_states:
            raise ValueError(f"update_task_state: state must be one of {valid_states}, got {state!r}")
        nlog.warn(f"Setting task {task_id} state → {state} (dry_run={dry_run})")
        result = await _t(connector.update_task_instance, dag_id, dag_run_id, task_id, state, dry_run=dry_run)
        nlog.ok(f"Task state updated to {state}")
        return {"dag_id": dag_id, "dag_run_id": dag_run_id, "task_id": task_id, "state": state, **result}

    elif operation == "clear_task":
        if not dag_id: raise ValueError("clear_task requires dag_id")
        dag_run_id = props.get("dag_run_id", "")
        task_id    = props.get("task_id", "")
        dry_run    = bool(props.get("dry_run", False))
        if not dag_run_id: raise ValueError("clear_task requires dag_run_id")
        if not task_id:    raise ValueError("clear_task requires task_id")
        nlog.info(f"Clearing task {task_id} (dry_run={dry_run})")
        result = await _t(connector.clear_task_instance, dag_id, dag_run_id, task_id, dry_run=dry_run)
        nlog.ok(f"Task cleared")
        return {"dag_id": dag_id, "dag_run_id": dag_run_id, "task_id": task_id, "cleared": True, **result}

    elif operation == "get_task_log":
        if not dag_id: raise ValueError("get_task_log requires dag_id")
        dag_run_id = props.get("dag_run_id", "")
        task_id    = props.get("task_id", "")
        try_number = int(props.get("try_number", 1))
        all_tries  = bool(props.get("all_tries", False))
        if not dag_run_id: raise ValueError("get_task_log requires dag_run_id")
        if not task_id:    raise ValueError("get_task_log requires task_id")
        nlog.info(f"Fetching log for task {task_id} try={try_number}")
        if all_tries:
            logs = await _t(connector.list_task_logs, dag_id, dag_run_id, task_id)
            nlog.ok(f"Fetched {len(logs)} try log(s)")
            return {"dag_id": dag_id, "dag_run_id": dag_run_id, "task_id": task_id, "logs": logs}
        else:
            log = await _t(connector.get_task_log, dag_id, dag_run_id, task_id, try_number)
            nlog.ok(f"Log: {len(log)} chars")
            nlog.raw(log[:3000])
            return {"dag_id": dag_id, "dag_run_id": dag_run_id, "task_id": task_id,
                    "try_number": try_number, "log": log[:8000], "truncated": len(log) > 8000}
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        limit = int(props.get("limit", 100))
        nlog.info(f"Listing Airflow variables (limit={limit})")
        variables = await _t(connector.list_variables, limit=limit)
        nlog.ok(f"Found {len(variables)} variable(s)")
        for v in variables:
            nlog.info(f"  {v.get('key','?'):<30s}  = {str(v.get('value',''))[:60]}")
        return {"variables": variables, "count": len(variables)}

    elif operation == "get_variable":
        key = props.get("key", "")
        if not key: raise ValueError("get_variable requires key")
        nlog.info(f"Getting variable: {key!r}")
        var = await _t(connector.get_variable, key)
        nlog.ok(f"{key} = {str(var.get('value',''))[:100]}")
        return {"key": key, "value": var.get("value"), "description": var.get("description", "")}

    elif operation == "set_variable":
        key   = props.get("key", "")
        value = props.get("value", "")
        desc  = props.get("description", "")
        if not key: raise ValueError("set_variable requires key")
        nlog.info(f"Setting variable: {key!r} = {str(value)[:80]}")
        result = await _t(connector.set_variable, key, value, description=desc)
        nlog.ok(f"Variable {key!r} set")
        return {"key": key, "value": value, **result}

    elif operation == "delete_variable":
        key = props.get("key", "")
        if not key: raise ValueError("delete_variable requires key")
        nlog.warn(f"Deleting variable: {key!r}")
        status = await _t(connector.delete_variable, key)
        nlog.ok(f"Variable {key!r} deleted")
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        limit = int(props.get("limit", 100))
        nlog.info(f"Listing connections (limit={limit})")
        conns = await _t(connector.list_connections, limit=limit)
        nlog.ok(f"Found {len(conns)} connection(s)")
        for c in conns:
            nlog.info(f"  {c.get('connection_id','?'):<30s}  type={c.get('conn_type')}  host={c.get('host','')}")
        return {"connections": conns, "count": len(conns)}

    elif operation == "get_connection":
        conn_id = props.get("conn_id", "")
        if not conn_id: raise ValueError("get_connection requires conn_id")
        nlog.info(f"Getting connection: {conn_id}")
        conn = await _t(connector.get_connection, conn_id)
        nlog.ok(f"Connection {conn_id}: type={conn.get('conn_type')} host={conn.get('host')}")
        return conn

    elif operation == "create_connection":
        conn_id   = props.get("conn_id", "")
        conn_type = props.get("conn_type", "")
        if not conn_id:   raise ValueError("create_connection requires conn_id")
        if not conn_type: raise ValueError("create_connection requires conn_type")
        nlog.info(f"Creating connection: {conn_id} (type={conn_type})")
        result = await _t(connector.create_connection,
            conn_id=conn_id, conn_type=conn_type,
            host=props.get("host", ""), port=props.get("port"),
            login=props.get("login", ""), password=props.get("password", ""),
            schema=props.get("schema", ""), extra=props.get("extra", ""),
        )
        nlog.ok(f"Connection {conn_id} created")
        return result

    elif operation == "update_connection":
        conn_id = props.get("conn_id", "")
        if not conn_id: raise ValueError("update_connection requires conn_id")
        fields = {k: v for k, v in props.items()
                  if k not in ("credential", "operation", "conn_id") and v not in (None, "")}
        nlog.info(f"Updating connection {conn_id}: fields={list(fields.keys())}")
        result = await _t(connector.update_connection, conn_id, **fields)
        nlog.ok(f"Connection {conn_id} updated")
        return result

    elif operation == "delete_connection":
        conn_id = props.get("conn_id", "")
        if not conn_id: raise ValueError("delete_connection requires conn_id")
        nlog.warn(f"Deleting connection: {conn_id}")
        status = await _t(connector.delete_connection, conn_id)
        nlog.ok(f"Connection {conn_id} deleted")
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        limit = int(props.get("limit", 100))
        nlog.info(f"Listing pools (limit={limit})")
        pools = await _t(connector.list_pools, limit=limit)
        nlog.ok(f"Found {len(pools)} pool(s)")
        for p in pools:
            nlog.info(f"  {p.get('name','?'):<30s}  slots={p.get('slots')}  occupied={p.get('occupied_slots',0)}")
        return {"pools": pools, "count": len(pools)}

    elif operation == "get_pool":
        pool_name = props.get("pool_name", "")
        if not pool_name: raise ValueError("get_pool requires pool_name")
        nlog.info(f"Getting pool: {pool_name}")
        pool = await _t(connector.get_pool, pool_name)
        nlog.ok(f"Pool {pool_name}: slots={pool.get('slots')} occupied={pool.get('occupied_slots',0)}")
        return pool

    elif operation == "create_pool":
        pool_name = props.get("pool_name", "")
        slots     = int(props.get("slots", 1))
        desc      = props.get("description", "")
        if not pool_name: raise ValueError("create_pool requires pool_name")
        nlog.info(f"Creating pool {pool_name!r} with {slots} slot(s)")
        result = await _t(connector.create_pool, name=pool_name, slots=slots, description=desc)
        nlog.ok(f"Pool {pool_name} created")
        return result
        ...
        slots     = int(props.get("slots", 1))
        desc      = props.get("description")
        if not pool_name: raise ValueError("update_pool requires pool_name")
        nlog.info(f"Updating pool {pool_name!r} → {slots} slot(s)")
        result = await _t(connector.update_pool, name=pool_name, slots=slots, description=desc)
        nlog.ok(f"Pool {pool_name} updated")
        return result

    elif operation == "delete_pool":
        pool_name = props.get("pool_name", "")
        if not pool_name: raise ValueError("delete_pool requires pool_name")
        nlog.warn(f"Deleting pool: {pool_name}")
        status = await _t(connector.delete_pool, pool_name)
        nlog.ok(f"Pool {pool_name} deleted")
        return {"pool_name": pool_name, "deleted": True}

    # ══════════════════════════════════════════════════════════════
    # XCom operations
    # ══════════════════════════════════════════════════════════════

    elif operation == "get_xcom":
        if not dag_id: raise ValueError("get_xcom requires dag_id")
        dag_run_id = props.get("dag_run_id", "")
        task_id    = props.get("task_id", "")
        xcom_key   = props.get("xcom_key", "return_value")
        if not dag_run_id: raise ValueError("get_xcom requires dag_run_id")
        if not task_id:    raise ValueError("get_xcom requires task_id")
        nlog.info(f"Getting XCom: {dag_id}/{dag_run_id}/{task_id}[{xcom_key}]")
        entry = await _t(connector.get_xcom_entry, dag_id, dag_run_id, task_id, xcom_key)
        nlog.ok(f"XCom value: {str(entry.get('value',''))[:200]}")
        return {"dag_id": dag_id, "dag_run_id": dag_run_id, "task_id": task_id,
                "key": xcom_key, "value": entry.get("value"), "timestamp": entry.get("timestamp")}

    elif operation == "list_xcom":
        if not dag_id: raise ValueError("list_xcom requires dag_id")
        dag_run_id = props.get("dag_run_id", "")
        task_id    = props.get("task_id", "")
        limit      = int(props.get("limit", 20))
        if not dag_run_id: raise ValueError("list_xcom requires dag_run_id")
        if not task_id:    raise ValueError("list_xcom requires task_id")
        nlog.info(f"Listing XCom entries for {task_id}")
        entries = await _t(connector.list_xcom_entries, dag_id, dag_run_id, task_id, limit=limit)
        nlog.ok(f"Found {len(entries)} XCom entry/entries")
        for e in entries:
            nlog.info(f"  key={e.get('key','?')}  value={str(e.get('value',''))[:60]}")
        return {"dag_id": dag_id, "dag_run_id": dag_run_id, "task_id": task_id,
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        nlog.info("Running Airflow health check")
        result = await _t(connector.health_check)
        status = result.get("status", "unknown")
        if status == "ok": nlog.ok("Airflow is healthy ✓")
        else:              nlog.error(f"Airflow health: {status}")
        return result

    elif operation == "get_version":
        nlog.info("Getting Airflow version")
        result = await _t(connector.get_version)
        nlog.ok(f"Version: {result.get('version')}")
        return result

    elif operation == "get_config":
        section = props.get("section")
        nlog.info(f"Getting Airflow config{' section=' + section if section else ''}")
        result = await _t(connector.get_config, section=section)
        nlog.ok("Config retrieved")
        return result

    # ══════════════════════════════════════════════════════════════
    # TRIGGER (default — full existing behaviour, backward compat)
    # ══════════════════════════════════════════════════════════════
        ...
        if not dag_id: raise ValueError("trigger requires dag_id")

        nlog.info(f"DAG: {dag_id}  wait={wait_for_completion}  preflight={wait_if_running}  timeout={timeout}s")
        ...
        if wait_if_running:
            ...
            try:
                ...
                if waited: nlog.ok(f"Pre-flight: {len(waited)} prior run(s) finished")
                else:      nlog.info("Pre-flight: DAG idle, triggering now")
            except Exception as pf_err:
                nlog.warn(f"Pre-flight check failed (continuing): {pf_err}")
        ...
        if wait_for_completion:
            ...
            nlog.info(f"Not waiting for completion — state: {final_state}")
            ...
            for pattern in validate_task_patterns:
                ...
                if all_passed:
                    ...
                    if props.get("assert_task_pattern_success", False):
                        raise AssertionError(f"Tasks matching {pattern!r} failed: {failed}")
        ...
        raise ValueError(
            f"Unknown Airflow operation: {operation!r}. Valid operations: "
            "trigger, list_dags, get_dag, get_dag_details, get_dag_source, "
            "pause, unpause, delete_dag, list_dag_warnings, "

...
User: "show me all dags"
Response: {{"operation": "list_dags", "params": {{}}, "explanation": "Listing all DAGs in Airflow"}}

User: "pause the daily_report dag"
Response: {{"operation": "pause_dag", "params": {{"dag_id": "daily_report"}}, "explanation": "Pausing the daily_report DAG to prevent scheduled runs"}}

User: "get the logs for task extract in dag etl_pipeline run manual__2024-01-01"
Response: {{"operation": "get_task_logs", "params": {{"dag_id": "etl_pipeline", "dag_run_id": "manual__2024-01-01", "task_id": "extract"}}, "explanation": "Fetching logs for the extract task"}}

User: "create a variable called api_key with value abc123"
Response: {{"operation": "create_variable", "params": {{"key": "api_key", "value": "abc123"}}, "explanation": "Creating a new Airflow variable"}}

User: "clear all failed tasks in the data_sync dag"
Response: {{"operation": "clear_task_instances", "params": {{"dag_id": "data_sync", "only_failed": true}}, "explanation": "Clearing all failed task instances in data_sync DAG for retry"}}
...
def wait_for_dags_to_complete(dag_infos, logs, execution_id, storage):
    ...
    while elapsed < max_wait_time:
        ...
        for dag_info in dag_infos:
            ...
            running_states = ['running', 'queued', 'scheduled']
            ...
            if state.lower() in running_states:
                ...
                logs.append({
                    ...
                    'level': 'INFO',
                    'message': f"DAG {dag_id} is currently {state}. Waiting..."
                })
    ...
    logs.append({
        'timestamp': datetime.now().isoformat(),
        'level': 'ERROR',
        'message': f"Timeout waiting for DAGs to complete after {max_wait_time} seconds"
    })
    return False
...
def execute_workflow_async(execution_id, workflow_id):
    ...
    if dag_infos:
        logs.append({'timestamp': datetime.now().isoformat(), 'level': 'INFO', 'message': f"Found {len(dag_infos)} DAG(s) to check: {', '.join([d['dag_id'] for d in dag_infos])}"})
        storage.update_execution(execution_id, 'checking', logs)
        ...
        logs.append({'timestamp': datetime.now().isoformat(), 'level': 'INFO', 'message': 'All DAGs are in non-running state. Proceeding with workflow execution.'})

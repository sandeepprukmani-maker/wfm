...
VALID NODE TYPES AND THEIR REQUIRED PROPS:
  ...
  airflow      → credential (required)
                 operation (default: "trigger") — one of:
                   DAG:      list_dags, get_dag, get_dag_details, get_dag_source,
                             pause, unpause, delete_dag, list_dag_warnings
                   Run:      trigger, list_runs, get_run, clear_run,
                             delete_run, update_run_state
                   Task:     get_task, update_task_state, clear_task, get_task_log
                   Variable: list_variables, get_variable, set_variable, delete_variable
                   Conn:     list_connections, get_connection, create_connection,
                             update_connection, delete_connection
                   Pool:     list_pools, get_pool, create_pool, update_pool, delete_pool
                   XCom:     get_xcom, list_xcom
                   System:   health_check, get_version, get_config

                 trigger-specific props: dag_id, wait_for_completion (bool), timeout (int),
                   conf (dict), wait_if_running (bool),
                   validate_task_patterns (list), assert_no_failed_tasks (bool)
                 run-specific props: dag_run_id, state, dry_run
                 task-specific props: dag_run_id, task_id, state, try_number, all_tries
                 variable props: key, value, description
                 connection props: conn_id, conn_type, host, port, login, password, schema, extra
                 pool props: pool_name, slots, description
                 xcom props: dag_run_id, task_id, xcom_key

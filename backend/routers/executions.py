...
# ── Bulk Execution (Sprint 3) ─────────────────────────────────────────────────

class BulkExecuteRequest(BaseModel):
    workflow_ids: list      # list of workflow IDs to run
    triggered_by: str = "manual"
    input_data:   dict = {}
...
async def bulk_run_workflows(
    ...
    """
    Fire multiple workflows concurrently.
    Returns immediately with a list of (workflow_id, run_id) pairs.
    Each workflow executes in its own background task.
    max_parallel caps how many actually run at the same time.
    """
    if not body.workflow_ids:
        raise HTTPException(400, "workflow_ids must not be empty")
    if len(body.workflow_ids) > 50:
        raise HTTPException(400, "Maximum 50 workflows per bulk run")

    queued = []
    for wf_id in body.workflow_ids:
        wf = db.query(Workflow).filter_by(id=wf_id, owner_id=current_user["sub"]).first()
        if not wf:
            queued.append({"workflow_id": wf_id, "run_id": None, "error": "not found"})
            continue

        run_id = f"run-{str(uuid.uuid4())[:8]}"
        run    = WorkflowRun(
            id=run_id, workflow_id=wf_id,
            triggered_by=body.triggered_by,
            trigger_data=body.input_data,
            status="pending",
            started_at=datetime.utcnow(),
        ...
        db.add(run)
        db.commit()
        queued.append({"workflow_id": wf_id, "workflow_name": wf.name, "run_id": run_id})

    # Fire all valid runs concurrently in a single background task
    valid = [q for q in queued if q.get("run_id")]
    if valid:
        background_tasks.add_task(
            _bulk_execute_bg,
            runs=valid,
    ...
    return {
        "queued":  len(valid),
        "skipped": len(queued) - len(valid),
        "runs":    queued,
        "message": f"{len(valid)} workflow(s) queued for parallel execution.",
    }


async def _bulk_execute_bg(runs: list, owner_id: str, max_parallel: int = 10):
    """Run all workflows concurrently, respecting max_parallel with a semaphore."""
    import asyncio
    from database import SessionLocal
    from credential_manager import CredentialManager
    from workflow_engine import WorkflowEngine
    ...
    async def _one(wf_id: str, run_id: str):
        async with sem:
            db = SessionLocal()
            try:
                mgr = CredentialManager(db)
    ...
    await asyncio.gather(*[_one(r["workflow_id"], r["run_id"]) for r in runs])

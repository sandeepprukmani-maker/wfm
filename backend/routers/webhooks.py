...
"""
FlowForge — Webhooks Router (v3, Sprint 3)

Sprint 3 additions:
  - HMAC-SHA256 signature validation (X-Hub-Signature-256 / X-FlowForge-Signature)
  - Token auth fallback (Authorization: Bearer <token>)
  - Signature secret stored in WebhookTrigger.auth_token as "hmac:<secret>"
"""
...
async def receive_webhook(
    ...
    # ── Authentication ────────────────────────────────────────────────────────
    if hook.auth_token:
        auth_val = hook.auth_token  # stored as "hmac:<secret>", "bearer:<token>", or plain token

        if auth_val.startswith("hmac:"):
            # HMAC-SHA256 signature validation
            # Supports: X-Hub-Signature-256 (GitHub style) or X-FlowForge-Signature
            secret      = auth_val[5:].encode()
            body_bytes  = await request.body()
            expected_sig = "sha256=" + hmac.new(secret, body_bytes, hashlib.sha256).hexdigest()
            provided_sig = (
                request.headers.get("X-Hub-Signature-256") or
                request.headers.get("X-FlowForge-Signature") or ""
            )
            if not hmac.compare_digest(provided_sig, expected_sig):
                logger.warning(f"Webhook {path!r}: HMAC signature mismatch")
                raise HTTPException(401, "Invalid webhook signature")
            logger.info(f"Webhook {path!r}: HMAC signature verified ✓")
            ...
            if not hmac.compare_digest(provided, auth_val):
